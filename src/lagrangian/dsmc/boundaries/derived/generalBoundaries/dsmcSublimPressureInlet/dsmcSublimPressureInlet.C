...
#include "dsmcSublimPressureInlet.H"
...
defineTypeNameAndDebug(dsmcSublimPressureInlet, 0);

addToRunTimeSelectionTable(dsmcGeneralBoundary, dsmcSublimPressureInlet, dictionary);
...
dsmcSublimPressureInlet::dsmcSublimPressureInlet
(
    Time& t,
    const polyMesh& mesh,
    dsmcCloud& cloud,
...
:
    ...
    inletPressure_(),
    inletTemperature_(),
    inletNumberDensity_(faces_.size(), scalar(0.0)),
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void dsmcSublimPressureInlet::initialConfiguration()
{}

void dsmcSublimPressureInlet::calculateProperties()
{}

void dsmcSublimPressureInlet::controlParcelsBeforeMove()
{   
...
//             
        ...
        if(infoCounter_ >= cloud_.nTerminalOutputs())
        {
            if (Pstream::parRun())
            {
                ...
                Info<< "dsmcSublimPressureInlet specie: " << typeIds_[iD]
...
//     {
//         Pout<< "dsmcSublimPressureInlet target parcels to insert: " << nTotalParcelsToBeAdded 
//             <<", number of inserted parcels: " << nTotalParcelsAdded
//             << endl;
//     }
}

void dsmcSublimPressureInlet::controlParcelsBeforeCollisions()
{
    
}

void dsmcSublimPressureInlet::controlParcelsAfterCollisions()
{    
    nTimeSteps_ += 1.0;
...
//             scalar faceNormalVelocity = (n & inletVelocity_[c]);
            ...
            inletNumberDensity_[c] = (inletPressure_ - pressure[c]) / (physicoChemical::k.value()*inletTemperature_);            
...
//     if(faces_.size() > VSMALL)
//     {
//         Pout << "dsmcSublimPressureInlet inlet velocity correction = " << velocityCorrection[(faces_.size()/2)] << endl;
//         Pout << "dsmcSublimPressureInlet inlet velocity = " << inletVelocity_[(faces_.size()/2)] << endl;
//     }
...
//             /(2.0*sqrtPi*cloud_.nParticle());      
            ...
            if(cloud_.axisymmetric())
            {
                ...
                moleFractions_[iD]*
                (
                    fA*inletNumberDensity_[f]*deltaT*mostProbableSpeed
                    *
                    (
                        exp(-sqr(sCosTheta)) + sqrtPi*sCosTheta*(1 + erf(sCosTheta))
                    )
...
void dsmcSublimPressureInlet::output
(
...
void dsmcSublimPressureInlet::updateProperties(const dictionary& newDict)
{
...
void dsmcSublimPressureInlet::setProperties()
{
    ...
    if(molecules.size() == 0)
    {
        FatalErrorIn("dsmcSublimPressureInlet::setProperties()")
    ...
    forAll(moleculesReduced, i)
    {
        ...
        if(typeId == -1)
        {
            FatalErrorIn("dsmcSublimPressureInlet::dsmcSublimPressureInlet()")
